...
for iterations in iteration_values:
    for features in feature_values:
        ...
        valid_recs, mean_ndcg = evaluate_with_recall('ALS', algo_als, downsampled_train_data, validation_data)
...
for result in results:
    print(f"Features = {result['Features']}, Iterations = {result['Iterations']}: Mean nDCG = {result['Mean Recall']:.4f}")
...
final_recs, mean_ndcg = evaluate_with_recall('Bias', final_algo, downsampled_train_data, final_test_data)

print(f"Recall mean for test set: {mean_ndcg:.4f}")

#################################################
ndcg_value = mean_ndcg
key_name = "biasedmf_goodreads_poetry_recall10"
