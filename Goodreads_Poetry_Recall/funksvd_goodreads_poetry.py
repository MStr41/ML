...
# Perform hyperparameter tuning on the validation set and compute Recall (Other hyperparameters have alredy been tested and tuned for the best configuration)
results = []
best_features = None
...
for iterations in iteration_values:
    for features in feature_values:
        ...
        # Evaluate the model and compute mean Recall
        valid_recs, mean_ndcg = evaluate_with_recall('FunkSVD', algo_funksvd, downsampled_train_data, validation_data)
        results.append({'Features': features, 'Iterations': iterations, 'Mean Recall': mean_ndcg})
...
for result in results:
    print(f"Features = {result['Features']}, Iterations = {result['Iterations']}: Mean Recall = {result['Mean Recall']:.4f}")

print(f"\nBest Features: {best_features}, Best Iterations: {best_iterations} (Mean Recall = {best_mean_ndcg:.4f})")
...
final_recs, mean_ndcg = evaluate_with_recall('FunkSVD', final_algo, downsampled_train_data, final_test_data)

print(f"Recall mean for test set: {mean_ndcg:.4f}")

#################################################
ndcg_value = mean_ndcg
key_name = "funksvd_goodreads_poetry_recall10"
